        ...
        if nome_cliente in lista_de_clientes:
            ...
            if cidade_origem == destino:
                if diarias_cliente > qt_dias_realizado: #se devolver na mesma cidade n tem custo adicional
                    ...
                    valor_total = (locacao_cliente.valor_diarias(diarias_cliente) + locacao_cliente.valor_km_rodado(km_rodado) * valor_km) - valor_desconto
                    valor_total -= valor_desconto
                    print(f"Valor total a pagar é R$ {valor_total:.2f}")
                    locacao_cliente.set_valor_total(valor_total)
                    input("Pressione ENTER para continuar")
                    ...
                    valor_total = (locacao_cliente.valor_diarias(diarias_cliente) + locacao_cliente.valor_km_rodado(km_rodado))
                ...
                if diarias_cliente > qt_dias_realizado:
                    valor_desconto = (diarias_cliente - qt_dias_realizado) * (20/100) #Caso tenha devolvido o carro antes, o sistema dá um desconto de 20% sobre as diárias não utilizadas
                    valor_total = (locacao_cliente.valor_diarias(diarias_cliente) + locacao_cliente.valor_km_rodado(km_rodado))
                    valor_total += locacao_cliente.valor_roaming(valor_total)
                    valor_total -= valor_desconto
                    print(f"Valor total a pagar é R$ {valor_total:.2f}")
                    locacao_cliente.set_valor_total(valor_total)
                    input("Pressione ENTER para continuar")
    ...
    def __init__(self, linha, veiculo):
        ...
        if self._destino != '':
            ...
            if self._origem != self._destino:
                self.valor_roaming(self._valor_total)
                self._valor_total += self._roaming
