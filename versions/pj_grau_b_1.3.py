    ...
    match(escolha):
        ...
        case 3:
             ...
             if nome_cliente in lista_de_clientes:
                print("O nome informado é valido")  
                destino= str(input('Insira a cidade onde o carro foi entregue: ')).title()
                km_rodado = int(input("Informe os km  rodados: ")) 
                veiculo_alugado = int(input("informe o numero do Veiculo: "))   #coloquei isso so pq precisa para pegar o valor do km e valor diaria
                for locacao in locacoes:
                    if locacao.get_cliente() == nome_cliente:
                        locacao_cliente = locacao
                        break

                #aqui tentei pegar o valor das diarias para calcular depois, mas n tem como pegar pelo nome do cliente acho
                for veiculo in veiculos:
                    if veiculo.get_veiculo() == veiculo_alugado:
                        valor_diaria= veiculos
                        valor_km= veiculos
                        break

                diarias_cliente = locacao_cliente.get_qt_dias_reserva()
                print("os dias reservados foram:", diarias_cliente, "dias")
                valor_diarias = valor_diaria.get_valores_diarias()
                valor_kms = valor_km.get_alor_km_rodado()
                qt_dias_realizado = int(input("Informe por quantos dias ficou com o carro alugado: "))  
                
                #aqui n esta dando print, n consegui arrumar
                 
                if diarias_cliente > qt_dias_realizado and cidade_origem == destino:#se devolver na mesma cidade n tem custo adicional
                    ...
                    valor_total = valor_desconto - valor_diarias + (valor_kms*km_rodado)
                ...
                if diarias_cliente < qt_dias_realizado and cidade_origem == destino:
                    valor_desconto = diarias_cliente - qt_dias_realizado * (30/100)
                    valor_total = valor_desconto + valor_diarias +(valor_kms*km_rodado) # tem que calcaular a taxa de roaming tambem                     
